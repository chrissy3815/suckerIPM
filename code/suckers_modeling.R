...
surv_alpha <- 220
surv_beta <- -20
fitted_surv<- function(z) {
  surv_min + (surv_max - surv_min) /
    (1 + exp(surv_beta * (log(z) - log(surv_alpha))))
}

# plot:
plot(ws_surv_points$len, ws_surv_points$surv, xlim = c(0,500), main )
exes<- 1:600
ws_whys<- ws_fitted_surv(exes)
four_whys<- fitted_surv(exes)
#lines(exes, ws_whys)
...
# point estimates from literature
ws_matur_points<- data.frame(len=c(160, 202, 210, 230, 260, 490),
