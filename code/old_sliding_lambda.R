...
#ss_fitTypical_wsdata<-nls(ss_vbTypical,data=ws_data_forfitting, start=list(K=ss_vbStarts$K, t0=-3))
...
#whys_wsdata<- ss_vbStarts$Linf*(1-exp(-coef(ss_fitTypical_wsdata)[1]*(exes-coef(ss_fitTypical_wsdata)[2])))
lines(exes, whys, lwd = 3)
#lines(exes, whys_wsdata, lwd = 3, lty = 2)
...
ss_m_par <- list(
  ...
  surv_mid = ss_surv_mid,
  surv_max = ss_surv_max,
  surv_alpha = ss_surv_alpha,
  surv_alpha2 = ss_surv_alpha2,
  surv_beta = ss_surv_beta,
  surv_beta2 = ss_surv_beta2,
